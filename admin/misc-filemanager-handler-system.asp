...
if Len(pf_id) > 0 then
	...
	If fso.FolderExists(strRootFolder & "/media/catalog/system/" & pf_id) Then
	Else
		fso.CreateFolder(strRootFolder & "/media/catalog/system/" & pf_id)
	End If
end if

savefolder=Server.MapPath("../media/catalog/system/" & pf_id)
...
If Request.Form("guidlist")&""<>"" Then
	...
	For i=0 to Ubound(list)
		...
		pstr = "FileURL=" & g_store_url & "media/catalog/system/" & pf_id & "/" & mvcfile.FileGuid & "-" & mvcfile.FileName
		
		dim xmlRecv2
		set xmlRecv2 = Server.CreateObject("Msxml2.ServerXMLHTTP")
		xmlRecv2.open "POST", "http://app.internetreports.com/api/upload?FileURL=" & g_store_url & "media/catalog/system/" & pf_id & "/" & mvcfile.FileGuid & "-" & mvcfile.FileName, False
